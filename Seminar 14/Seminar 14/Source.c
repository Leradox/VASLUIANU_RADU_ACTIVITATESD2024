...
void afisareCuParguregereInLatime(NodPrincipal* graf, int nodPlecare) {
	//initializari
	NodCoada* coada = NULL;
...
void afisareCuParguregereInAdancime(NodPrincipal* graf, int nodPlecare) {
	//initializari
	NodCoada* stiva = NULL;
	int nrNoduri = getNumarNoduri(graf);
	char* vizitate = (char*)malloc(sizeof(char) * nrNoduri);
	...
	while (stiva) {
		//extragem nodul din coada
		int idNodCurent = extragereDinStiva(&stiva);
		NodPrincipal* nodCurent = cautaLinie(graf, idNodCurent);
		afisareAutobuz(nodCurent->info);
		NodSecundar* temp = nodCurent->vecini;
		//inserare in coada si marcare ca vizitat fiecare vecin
...
void main() {
	...
	//stiva afisare in adancime
	//coada (first in first out) afisare in latime

	afisareCuParguregereInLatime(graf, 0);
